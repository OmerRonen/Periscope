...
from .globals import (C_MAP_PRED, UPPER_TRIANGULAR_MSE_LOSS,
                     UPPER_TRIANGULAR_CE_LOSS, CONTACT_MAP_PH, MODELS_PATH,
                     PROTEIN_BOW_DIM)
...
def _compute_sequence_distance_mat_np(l):
    l_int = int(l)
...
def _compute_sequence_distance_mat(l):
    sequence_distance_ind_mat = tf.numpy_function(
        _compute_sequence_distance_mat_np, [l], tf.float32)
...
class NetParams:
    ...
    def load(self):
        ...
        params['net']['data']['log_pssm'] = params['net']['data'].get('log_pssm', True)
        ...
        params['net']['data']['resnet_features'] = params['net']['data'].get(
            'resnet_features', None)

        params['train']['batch_size'] = params['train'].get('batch_size', 1)

        if 'features' in params['net'][
            'data'] and 'conv_features' not in params['net']['data']:
        ...
        net = {
            'architecture': {
                ...
                'references_resnet': {
                    ...
                    'num_res_filters': num_res_filters
                },
                'arch': arch,
            },
            'data': {
...
def multi_structures_op(dms, seq_refs, seq_target, plmc, conv_params, ccmpred=None, k=30, prot_dim=PROTEIN_BOW_DIM):
    """Multi structures contact map prediction operation

    Args:
        dms (tf.Tensor): reference distance matrix of shape (1, l, l, None)
        seq_ref (tf.Tensor): reference sequence of shape (1, l, PROTEIN_BOW_DIM, None)
        seq_target (tf.Tensor): target sequence of shape (1, l , PROTEIN_BOW_DIM)
        plmc (tf.Tensor): plmc energy matrix of shape (1, l, l, 1)
        conv_params (dict): parameters to the last deep conv layer
        ccmpred (tf.Tensor): plmc energy matrix of shape (1, l, l, 1)
...
def multi_structures_op_2(dms, seq_refs, seq_target, plmc, conv_params,ccmpred=None, k=30, prot_dim=PROTEIN_BOW_DIM):
    """Multi structures contact map prediction operation
...
def _single_structure_op(dm,
                         ...
                         output_dim=1):
    """Performes a deep operation on a single structure to obtain a tensor to be integrated with other structures
    ...
    Args:
        ...
        k (int): dimension to project the sequence to
        output_dim (int): last dimention of the output
    ...
    evo_coupling_arr = plmc if ccmpred is None else tf.concat([plmc, ccmpred], axis=3)
...
def pairwise_feature(dim, feature_name):
    """Input to pairwise predictor

    Args:
        feature_name (str): the name of the feature
        dim (int): the number of conv_features

    Returns:
        tf.Tensor: input tensor to the model

    """
    inp = tf.placeholder(tf.float32, [None, None, dim], name=feature_name)
    return inp


def get_true_cm_ph():
    """True contact map to a predictor

    Returns:
        tf.Tensor: input tensor to the model

    """
    cm = tf.placeholder(tf.float32, [None, None, 1], name=CONTACT_MAP_PH)
    return cm


def contact_map_predictor_ph(num_features):
    """Returns the place holders for the model input and output

    Args:
        num_features (int): number of input _features

    Returns:
        tuple[tf.Tensor, tf.Tensor]: input and output place holders

    """

    features = pairwise_feature(num_features)
    contact = get_true_cm_ph()

    return features, contact


def random_data_generator(input_shape):
    for i in range(100):
        np.random.seed(0)

        l = np.random.choice([3, 4, 5])
        x, y = np.random.random([l, l, input_shape
...
#
#
# data_generator = random_data_generator(input_shape=6)
...
def model_fn(features, labels, mode, params):
    ...
    contact_pred = deep_conv_op(conv_input_tensor=features)
    loss = upper_triangular_mse_loss(contact_pred, labels)

    if mode == tf.estimator.ModeKeys.PREDICT:
        return tf.estimator.EstimatorSpec(mode, predictions=contact_pred)
    else:
        if mode == tf.estimator.ModeKeys.EVAL:
            ...
            train_op = get_opt_op(loss)
            global_step = tf.train.get_global_step()
            global_step = tf.assign(global_step,
                                    global_step + 1,
...
def train_input_fn():
    shapes, types = ((None, None, 6), (None, None, 1)), (tf.float32,
                                                         tf.float32)
    dataset = tf.data.Dataset.from_generator(random_data_generator,
                                             args=[6],
                                             output_types=types,
