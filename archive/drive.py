...
GoogleAuth.DEFAULT_SETTINGS['client_config_file'] = os.path.join(
    periscope_path, 'client_secrets.json')
...
def _get_drive():
    gauth = GoogleAuth()
    gauth.LoadCredentialsFile(os.path.join(periscope_path, "mycreds.txt"))
    if gauth.access_token_expired:
        gauth.Refresh()
    else:
        gauth.Authorize()
    gauth.SaveCredentialsFile(os.path.join(periscope_path, "mycreds.txt"))
