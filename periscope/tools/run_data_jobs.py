...
def parse_args():
    parser = ArgumentParser(description="Generate data for a given dataset")
    ...
    for job in range(n_jobs-1):
        ...
        with tempfile.NamedTemporaryFile(suffix='.sh') as f:
            with open(f.name, 'w') as fh:
                ...
                fh.writelines("#SBATCH --job-name=%s_data\n" % job_name)
                fh.writelines(f"#SBATCH --output={os.path.join(PATHS.periscope, 'slurm_scripts', job_name + '.dataout')}\n")
                ...
                fh.writelines(f"python3 -m periscope.tools.prot_data_gen {' '.join(job_prots)}")
